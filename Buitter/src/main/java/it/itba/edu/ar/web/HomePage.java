	...
	public HomePage() {
		...
		emptyUserContainer.add(new ListView<User>("users", modelUsers) {
			...
			protected void populateItem(ListItem<User> item) {
				item.add(new Image("userImage", new ProfPicResourceReference(item.getModel().getObject().getThumbnailPhoto(), item.getModel().getObject().getUsername())));
		...
		whoToFollowContainer.add(new ListView<User>("recommendedUsers", modelRecommendations) {
			@Override
			protected void populateItem(ListItem<User> item) {
				item.add(new Image("recommendedUserImage", new ProfPicResourceReference(item.getModel().getObject().getThumbnailPhoto(), item.getModel().getObject().getUsername())));
				PageParameters pgParameters = new PageParameters();
				pgParameters.add("username", item.getModelObject().getUsername());
